...
const dxnoteReducer = produce((state: dxnoteTypes, action: any) => {
  switch (action.type) {
    case "switch_repo": {
      let dxnote_info = action.data_path
    ...
    case "switch_folder": {
      ...
      let cur_repo_key = action.repo_key
        ? action.repo_key
        : dxnote_info["cur_repo_key"];
      !dxnote_info["repos"] && (dxnote_info["repos"] = {});
    ...
    case "switch_note": {
      ...
      let cur_folder_key = action.folder_key
        ? action.folder_key
        : dxnote_info["repos"][cur_repo_key]["cur_folder_key"];
      dxnote_info["repos"][cur_repo_key]["folders"][cur_folder_key] =
        action.note_key;
...
export const useDxnote = () => {
  ...
  const currentRepoKey = useMemo(() => {
    let cur_repo_key = state["cur_repo_key"];
    console.log("cur_repo_key: " + cur_repo_key);
    return cur_repo_key;
  }, [state]);

  const currentFolderKey = useMemo(() => {
    let cur_folder_key = state.repos[state.cur_repo_key]?.cur_folder_key;
  ...
  const currentNoteKey = useMemo(() => {
    let cur_folder_key = state.repos[state.cur_repo_key]?.cur_folder_key;
    let cur_note_key = state.repos[state.cur_repo_key]?.folders[cur_folder_key];
