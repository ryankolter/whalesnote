...
const RepoList: React.FC<RepoListProps> = ({
  ...
  const preRepoPage = useCallback(() => {
    if (repoScrollPage > 0) {
      if (repoScrollRef && repoScrollRef.current) {
        repoScrollRef.current.scrollLeft =
          Math.floor((repoScrollPage - 1) / 5.0) *
          repoScrollRef.current.offsetWidth;
      }
                ...
                {repos_key
                  ...
                  .map((key, index) => {
                    if (index === repoScrollPage * 9) {
                      return (
                        <Sortable key={key} id={key} className="repoItemSort">
                          <RepoItem
                            key={key}
                  ...
                  {activeId && repos_obj ? (
                    <RepoItem key={activeId}>
                      <RepoItemName
                        style={{
                          backgroundColor:
                            currentRepoKey === activeId ? "#3a404c" : "",
                        }}
                      >
