...
function getClassRegex(className) {
  return (
    ...
    (CLASS_REGEX[className] = new RegExp("\\s*" + className + "(\\s*)", "g"))
  );
}
...
function switchMode(editor) {
  ...
  var isSideBySideActive = /editor-preview-active-side/.test(
    sidebyside.className
  );
  ...
  //     console.log("in edit mode , change to preview mode")
  //     togglePreview(editor)
  //     window.localStorage.setItem("view_mode", "preview")
  // }else if(isPreviewActive){
  //     //in preview mode , change to sidebyside mode
  //     console.log("in preview mode , change to sidebyside mode")
  //     toggleSideBySide(editor);
  //     window.localStorage.setItem("view_mode", "sidebyside")
  // }else if(isSideBySideActive){
  //     // in side by side mode , change to edit mode
  //     console.log("in side by side mode , change to edit mode")
  //     toggleSideBySide(editor);
...
EasyMDE.prototype.uploadImages = function (files, onSuccess, onError) {
  ...
  this.updateStatusBar(
    "upload-image",
    this.options.imageTexts.sbOnDrop.replace("#images_names#", names.join(", "))
  );
};
  ...
  this.codemirror = CodeMirror.fromTextArea(el, {
    ...
    styleSelectedText:
      options.styleSelectedText != undefined
        ...
        : !isMobile(),
    scrollbarStyle:
      options.scrollbarStyle != undefined ? options.scrollbarStyle : "native",
...
EasyMDE.prototype.uploadImage = function (file, onSuccess, onError) {
  ...
  request.onload = function () {
    ...
    if (
      ...
      onSuccess(
        (self.options.imagePathAbsolute ? "" : window.location.origin + "/") +
          response.data.filePath
      );
    } else {
...
EasyMDE.prototype.setPreviewMaxHeight = function () {
  ...
  if (this.options.maxHeight.includes("calc")) {
    var extraHeight =
      " + " + (paddingTop * 2 + borderTopWidth * 2).toString() + "px)";
